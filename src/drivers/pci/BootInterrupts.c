...
void (*video_interrupt_hook)(void) = 0;
ts_pm_interrupt IDT_LOC[256];
...
void BootInterruptsWriteIdt() {
	...
	idtr.len = sizeof(IDT_LOC)-1;
	idtr.offset = (uint32_t)IDT_LOC;
	dbg_load_idt(&idtr);
...
// this guy is getting called at 1kHz
...
{
	if (video_interrupt_hook) {
		video_interrupt_hook();
	} else {
