...
void PciWriteDword(unsigned int bus, unsigned int dev, unsigned int func, unsigned int reg_off, unsigned int dw)
{
	u32 base_addr = 0x80000000;
	base_addr |= ((bus & 0xFF) << 16);	// bus #
	base_addr |= ((dev & 0x1F) << 11);	// device #
...
void BootPciPeripheralInitialization()
{
	...
	PciWriteByte (0, 1,    0, 4,    1);
	PciWriteDword(0, 1,    0, 0x10, 0x8001);

	PciWriteByte (0, 1,    1, 4,    1);
	PciWriteDword(0, 1,    1, 0x14, 0xc001);
	PciWriteDword(0, 1,    1, 0x18, 0xc201);

	PciWriteDword(0, 0x1e, 0, 0x04, 0x00000007);
	PciWriteDword(0, 0x1e, 0, 0x18, 0x00010100);
	PciWriteDword(0, 0x1e, 0, 0x20, 0xfdf0fd00);
	PciWriteDword(0, 0x1e, 0, 0x24, 0xf7f0f000);

	PciWriteDword(1, 0,    0, 0x04, 7);
	PciWriteDword(1, 0,    0, 0x10, 0xfd000000);
	PciWriteDword(1, 0,    0, 0x14, 0xf0000000);
	...
	IoOutputByte(0xcf9, 0x0);	// Reset Port
        ...
        IoOutputByte(0x40, 0xA9);              // 1kHz (1.19318MHz/1193)
